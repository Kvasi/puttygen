...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	if(type == SSH_KEYTYPE_SSH1) {
		if(realtype == type)
			ret = loadrsakey(fn, &newkey1, passphrase, &errmsg);
		else
			ret = import_ssh1(fn, realtype, &newkey1,
				passphrase, &errmsg);
	}
	else {
		if(realtype == type)
			newkey2 = ssh2_load_userkey(fn, passphrase, &errmsg);
		else
			newkey2 = import_ssh2(fn, realtype, passphrase, &errmsg);
		if(newkey2 == SSH2_WRONG_PASSPHRASE)
			ret = -1;
		else if(!newkey2)
			ret = 0;
	...
	fn = filename_from_str(argv[1]);
	ret = ssh2_save_userkey(fn, newkey2, NULL);
	filename_free(fn);
